...
//  Schedules
//
//  THIS CLASS IS AN EXTERNAL LIBRARY.....
//
...
extension UIAlertController {
    ...
    convenience init(style: UIAlertController.Style, on viewController: UIViewController? = nil, title: String? = nil, message: String? = nil) {
        self.init(title: title, message: message, preferredStyle: style)
...
// MARK: Personal Extensions

extension UIAlertController {
    
    typealias SquadronPickerCompletion = (_ squadron: String) -> Void
    
    static func showSquadronPicker(values: [String], initialSelection: String?, completion: SquadronPickerCompletion?) {
        let alert = UIAlertController(style: .actionSheet, title: "Squadrons", message: "Select Squadron")
        let initialSelection: PickerViewController.Index = (column: 0, row: values.firstIndex(of: initialSelection!) ?? 0)
        
        alert.addPickerView(values: [values], initialSelection: initialSelection) { _, _, index, values in
            completion?(values[index.column][index.row])
        }
    ...
    static func showDatePicker(date: Date?, completion: DatePickerCompletion?) {
